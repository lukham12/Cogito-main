...
@export var maxPlayerDistance: float = 4;

@export var damageRange : float = 1;
@export var predatorWeight = 500;
...
func escapePredator():
	for boid in boids:
		...
		if playerDist < damageRange:
			...
			var dir = (playerRef.get_position() - boid.get_position()).normalized();
			
			var multiplier = sqrt(1 - (playerDist / maxPlayerDistance));
